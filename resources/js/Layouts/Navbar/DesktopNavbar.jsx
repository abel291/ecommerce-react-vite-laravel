...
export default function DesktopNavbar({ navigation }) {
	...
	const { auth, filters, categories, shoppingCart } = usePage().props
	const { url } = usePage()
	...
	return (
		<nav className="border-b  hidden lg:block">
			...
			<div className="container mx-auto pt-4  text-sm grid grid-cols-12 gap-x-5  items-center ">
				...
				<div className="flex gap-x-6 items-center font-medium relative xl:col-span-10 ">
					<div><CategoriesDropdown /></div>
					{categories.map((item) => (
						<Link key={item.slug} href={route('department', item.slug)}
							className={
								(url === route('search', { 'department': item.slug })
									? 'border-indigo-600 text-primary-600 '
									: 'border-transparent text-gray-900') +
								'border-transparent text-gray-900 whitespace-nowrap px-1 py-4 pb-3  font-medium block '}>
							{item.name}
						</Link>
					))}
				</div>
				<div className="flex gap-x-5 items-center ml-auto xl:col-span-2 ">
					{auth.user ? (
						<ProfileDropdown>
					...
					<Link href={route('shopping-cart.index')}>
						<div className='group -m-2 flex items-center p-2'>
							...
							<span className="ml-2 text-sm font-medium text-gray-700 group-hover:text-gray-800">{auth.shoppingCart}</span>
						</div>
					</Link>
				</div>
			</div>
