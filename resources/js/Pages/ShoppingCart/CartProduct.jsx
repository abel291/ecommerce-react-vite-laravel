...
const ProductsCart = ({ cardProduct }) => {
	const { data, setData, delete: destroy, put, processing, errors } = useForm({ quantity: cardProduct.quantity })
	...
	const handleClickRemoveItem = () => {
		destroy(route('shopping-cart.destroy', cardProduct.id), {
			preserveScroll: true,
		})
	}
	...
	useEffect(() => {
		...
		put(route('shopping-cart.update', cardProduct.id), {
			preserveScroll: true,
		})
			...
			<div className={" py-6 w-full relative "}>
				...
				<div className="grid grid-cols-12 gap-6 md:gap-4 items-stretch">
					<div className="col-span-5 md:col-span-2">
						<Link href={route('product', { slug: cardProduct.model.slug })}>
							<div className=" flex items-center justify-center h-full">
								<img className=" md:max-h-28 lg:max-h-28 max-w-full" src={cardProduct.model.img} alt={cardProduct.name} />
							</div>
						</Link>
					</div>
					<div className="col-span-7 md:col-span-5">
						<div className="flex flex-col  h-full gap-y-2">
							...
							<div>
								{cardProduct.options.attributes.map((attribute) => (
									<div className="flex gap-x-1.4 text-sm">
					...
					<div className=" col-span-6 md:col-span-2">
						<div>
							<select
								...
								value={data.quantity}
							>
								{[...Array(cardProduct.model.maxQuantity).keys()].map((i) => (
							...
							<div className=" text-gray-400 text-xs mt-2 space-y-1">
								...
								{cardProduct.quantity > 1 && (
									<div className=""> 1 x {formatCurrency(cardProduct.price)} </div>
								)}
							</div>
